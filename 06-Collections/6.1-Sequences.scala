object Sequences extends App {
  ...
  def one(numberOfFilms: Int): Seq[Director] = directors.filter(_.films.length > numberOfFilms)
  def two(year: Int): Option[Director]       = directors.find(_.yearOfBirth < year)

  def three(year: Int, numberOfFilms: Int): Seq[Director] =
    directors.filter(d => d.yearOfBirth < year && d.films.length > numberOfFilms)

  def four(ascending: Boolean = true): Seq[Director] =
